...
vector<vector<double> > opponent(int number_choices); //Set of costs created by the self-aware opponent
...
int main()
{
	...
	for (size_t i = 1; i < CHOICES; i++)
	{
		...
		for (size_t j = 0; j < number_choices; j++)
		{
			if (player_totals[i] < player_totals[min_index])
			{
				min_index = j;
			}
		}
		total_player_cost += costs[i][min_index];
		outs << "Round " << i << ": " << endl;
...
vector<vector<double> > opponent(int number_choices)
{
